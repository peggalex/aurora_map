...
import worldLowMap from "@amcharts/amcharts5-geodata/worldLow";
...
export function Chart(props) {
	...
	useLayoutEffect(() => {
		...
		slider.events.on("rangechanged", function () {
			updateDateNight(
				(slider.get("start", 0) - 0.5) *
					am5.time.getDuration("day", 2) +
					new Date().getTime()
			);
		});
		...
		function updateDateNight(time) {
			...
			nightDataItem0.set(
				"geometry",
				am5map.getGeoCircle(nightPosition, 92)
			);
			nightDataItem1.set(
				"geometry",
				am5map.getGeoCircle(nightPosition, 90)
			);
			nightDataItem2.set(
				"geometry",
				am5map.getGeoCircle(nightPosition, 88)
			);

			setNightDatas(
				[88, 90, 92].map((radius) => {
					const nightData = am5map.getGeoCircle(
						nightPosition,
						radius
					);
					const [oldCoords] = nightData.coordinates;

					const isPositiveLongitude = (coord) => coord[0] >= 0;

					const wrapIndex = oldCoords.findIndex((coord, i) => {
						return (
							isPositiveLongitude(coord) !==
								isPositiveLongitude(oldCoords.at(i - 1)) &&
							Math.abs(coord[0] - oldCoords.at(i - 1)[0]) > 100
						);
					});

					const beforeWrap = oldCoords.slice(0, wrapIndex);
					const afterWrap = oldCoords.slice(wrapIndex);

					const [startCoords, endCoords] = (() => {
						if (isPositiveLongitude(oldCoords[wrapIndex])) {
							return [
								[
									[180, -90],
									[180, beforeWrap.at(-1)[1]],
								],
								[
									[-180, afterWrap[0][1]],
									[-180, -90],
		...
		function solarPosition(time) {
			...
			var longitude =
				((am5.time.round(new Date(time), "day", 1).getTime() -
					time -
					offset) /
					864e5) *
					360 -
			...
			return am5map.normalizeGeoPoint({
				longitude:
					longitude - equationOfTime(centuries) * am5.math.DEGREES,
		...
		function solarApparentLongitude(centuries) {
			return (
				...
				(0.00569 +
					0.00478 *
						Math.sin(
							(125.04 - 1934.136 * centuries) * am5.math.RADIANS
						)) *
					am5.math.RADIANS
			);
		}
		...
		function solarGeometricMeanLongitude(centuries) {
			var l =
				(280.46646 +
					centuries * (36000.76983 + centuries * 0.0003032)) %
				360;
